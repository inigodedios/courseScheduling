...
def can_schedule_course_on_date(self, course, date, schedule, allowed_time_slots, course_occurrences):
    ...
    # METHOD IN WHICH If there are courses not assigned and there are dates with no classes scheduled, the classes remain unassigned
    ...
    #             courses_by_color[color] = []
    #         courses_by_color[color].append(course)
    ...
    #         # Randomly select a color group that fits the time of day
    #         color_groups = list(courses_by_color.keys())
    #         random.shuffle(color_groups)
    #         for color in color_groups:
    #             courses_today = courses_by_color[color]
    #             random.shuffle(courses_today)

    #             # Attempt to schedule each course in the color group
    #             for course in courses_today:
    #                 # Check if we still need to schedule more sessions for this course
    #                 if course_occurrences[course] < course_sessions[course]:
    #                     # Check if the course can be scheduled today based on its allowed time slots
    ...
    #         # Sort the day's schedule by time slot
    #         schedule[date].sort(key=lambda x: list(time_slots.values()).index(x[1]))
